...
public class TemplateController extends BaseController<TemplateController> {
	...
	public ModelAndView query() throws HbcsoftException {
		...
		final String formType = this.request.getParameter("formType");
		...
		fieldList.clear();
		fieldList = templateService.queryAll(lstFormFields, lstTn,formType);
	...
	public ModelAndView addSave() throws HbcsoftException {
		...
		lstFormFields.forEach(ff -> {
			...
			}else if(!"".equals(ff.getClickValue()))
			{
				ff.setClickValue(String.valueOf(companyId));
			...
			}else if(ff.getFieldName().equalsIgnoreCase("FORMTYPE"))
			{
				ff.setInputValue(formType);
			}else{
				setEditData(ff);
			}
		});
