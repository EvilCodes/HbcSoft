						...
						<tr ondblclick="viewReport('${reportConfigs.id }')">
							...
							<td>
								<c:forEach  items="${list}" var="tNavMenu">
									<c:if test="${reportConfigs.dataConfig==tNavMenu.dictCode}">${tNavMenu.dictName}</c:if>
								</c:forEach>
							</td>
							<td>
								<c:forEach  items="${dbLinkParaList}" var="dbLinkPara">
									<c:if test="${reportConfigs.dataSourceId==dbLinkPara.id}">${dbLinkPara.dbAccount}</c:if>
								</c:forEach>
							</td>
							<td>
