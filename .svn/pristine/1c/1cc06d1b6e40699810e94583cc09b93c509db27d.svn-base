...
public class DataDictQueryServiceImp extends
	...
	@Override
	public List<DataDict> getEnabledList(final String dictCode, final String companyId) throws HbcsoftException {
		List<DataDict> list = new ArrayList<DataDict>();
		final String sql = HbcsoftCache.getSqlValue("dataDict_getEnabledList");
		try {
			list = dataDictDao.queryAll(sql, jt,new Object[]{dictCode,companyId});
		} catch (InstantiationException e) {
			this.getLogger().info(e);
		} catch (IllegalAccessException e) {
			this.getLogger().info(e);
