...
<script type="text/javascript">
	...
	function TiJiao(){
		...
		if(reportName == ''){
			...
			alert("请填写基础sql！");
			return false;
		}else if(dataConfig == 1 && reportSQL == ''){
			alert("请填写存储过程名称！");
			return false;
	...
	function modifyDiv(){
		...
		/* var val=document.getElementById("dataConfig").value;
		if(val==1){
			$("label#typeName").html("存储过程名称");
			document.getElementById("listSqlDiv").style.display = 'block';
			document.getElementById("reportSqlDiv").style.display = 'none'; 
		}else if(val==0){
			
			document.getElementById("listSqlDiv").style.display = 'none';
			document.getElementById("reportSqlDiv").style.display = 'block';
		}else{
							...
							<div class="col-md-10 col-sm-8">
								<select class="form-control" id="dataConfig" name="dataConfig" onchange="modifyDiv()">
									<c:forEach items="${list}" var="tNavMenu">
										<option value="${tNavMenu.dictCode}">${tNavMenu.dictName}</option>
									</c:forEach>
								
									<!-- <option value="0">基础sql</option>
									<option value="3">特殊格式</option> -->
								</select>
							</div>
						</div>
						<div class="form-group col-md-6 col-sm-6" id = "reportSqlDiv" name ="reportSqlDiv">
							<label for="drive" class="control-label col-md-2 col-sm-4" id="typeName">基础SQL</label>
							<div class="col-md-10 col-sm-8">
								<input type="text" class="form-control" name="reportSQL" id="reportSQL">
							</div>
						</div>
						<div class="form-group col-md-6 col-sm-6">
							...
							<div class="col-md-10 col-sm-8">
								<select class="form-control" id="dataSourceId" name="dataSourceId">
									<c:forEach items="${dbLinkParaList}" var="dbLinkPara">
										<option ${dbLinkPara.id ==1?'selected':''}  value="${dbLinkPara.id}">${dbLinkPara.dbAccount}</option>
									</c:forEach>
								</select>
							</div>
						</div>
