...
public class ReportController extends BaseController<ReportController> {
	...
	private List<ReportSpecialRow> lstReportSpecialRow = new ArrayList<ReportSpecialRow>();
	private List<ReportSpecialCol> lstReportSpecialCol = new ArrayList<ReportSpecialCol>();
	...
	private String sepcialRow = "";
	private String sepcialCol = "";
	...
	public String getExcelDemo() throws UnsupportedEncodingException, HbcsoftException, ParseException {
		...
		ec.setSpecialRow(this.sepcialRow);
		ec.setSpecialColumn(this.sepcialCol);
	...
	public List<ReportSpecialRow> getLstReportSpecialRow() {
		return lstReportSpecialRow;
	}

	public void setLstReportSpecialRow(List<ReportSpecialRow> lstReportSpecialRow) {
		this.lstReportSpecialRow = lstReportSpecialRow;
	}

	public List<ReportSpecialCol> getLstReportSpecialCol() {
		return lstReportSpecialCol;
	}

	public void setLstReportSpecialCol(List<ReportSpecialCol> lstReportSpecialCol) {
		this.lstReportSpecialCol = lstReportSpecialCol;
	}
	 ...
	 * @throws HbcsoftException 
	 */
	private void getSpecialRow() throws HbcsoftException {
		// TODO Auto-generated method stub
		this.getSpecialRowConfig();
		for (final Iterator<ReportSpecialRow> iter = lstReportSpecialRow.iterator(); iter.hasNext();) {
			final ReportSpecialRow reportSpecialRow = (ReportSpecialRow) iter.next();
			if(reportSpecialRow.getSpecialRow() != 0){
				this.sepcialRow += reportSpecialRow.getSpecialRow()+",";
			}
		}
		this.sepcialRow = 
				"".equals(this.sepcialRow) ? "":this.sepcialRow.substring(0, this.sepcialRow.length()-1);
	}
	/**
	 * 获取特殊列字符串
	 * @throws HbcsoftException 
	 */
	private void getSpecialCol() throws HbcsoftException {
		// TODO Auto-generated method stub
		this.getSpecialColConfig();
		for (final Iterator<ReportSpecialCol> iter = lstReportSpecialCol.iterator(); iter.hasNext();) {
			final ReportSpecialCol reportSpecialCol = (ReportSpecialCol) iter.next();
			if(reportSpecialCol.getSpecialColumn() != 0){
				this.sepcialCol += reportSpecialCol.getSpecialColumn()+",";
			}
		}
		this.sepcialCol = 
				"".equals(this.sepcialCol) ? "":this.sepcialCol.substring(0, this.sepcialCol.length()-1);
	}
	private void getSpecialRowConfig() throws HbcsoftException {
		// TODO Auto-generated method stub
		this.lstReportSpecialRow = reportSpecialRowService.findByReportConfigId(
				this.reportConfig.getId(),this.companyId);
	}
	private void getSpecialColConfig() throws HbcsoftException {
		// TODO Auto-generated method stub
		this.lstReportSpecialCol = reportSpecialColService.findByReportConfigId(
				this.reportConfig.getId(),this.companyId);
	}
