		...
		$(".pagination").jBootstrapPage({
			...
			onPageClicked: function(obj, pageIndex) {
			 ...
			 $.ajax({
					...
					data: 'json',
					async:true,
					url: '${pageContext.request.contextPath}/reportConfig/pageList.hbc?currentPage='+(pageIndex+1)+"&searchTable="+$("#searchTable").val()+"&searchMemo="+$("#searchMemo").val(),
					success: function(msg){
						...
						if (ttt.interfaceList.length == 0){
							for (var i = 0; i < ttt.list.length;i++){
								var tt = ttt.list[i];
								$("#num_tr").append(
										"<tr ondblclick='editTable(\""+tt.id+"\")'>"+
										"<td>"+
											"<input type='checkbox' name='rowid' value='"+tt.id +"'>"+
										"</td>"+
										"<td>"+
											(pageIndex*10+i+1)+
										"</td>"+
							...
							for (var i = 0; i < ttt.list.length; i++){
								var flag = false;
								var tt = ttt.list[i];
								for (var j = 0; j < ttt.interfaceList.length; j++){
									if (tt.id == ttt.interfaceList[j].primaryListId){
										flag = true;
									}
								}
								if (flag){
									$("#num_tr").append(
											"<tr ondblclick='editTable(\""+tt.id+"\")'>"+
											"<td>"+
												"<input type='checkbox' name='rowid' value='"+tt.id +"' checked='checke'>"+
											"</td>"+
											"<td>"+
											(pageIndex*10+i+1)+
											"</td>"+
