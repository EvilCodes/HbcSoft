    		...
    		function loadData(dd){
        		...
        		formFields.each(function(){ 
        			...
        			if (!rr.length) { 
        				...
        				var fTextbox = form.find("input[textboxName=\"" + name + "\"]"); 
        				if (f.length) { 
        					f.numberbox("setValue", value); 
        				}else if(fTextbox.length){
        					fTextbox.textbox("setValue", value);
        				}else{
...
$.extend($.fn.datagrid.methods,{
	statistics:function(jq){
		...
		var forzenOpt=$(jq).datagrid('options').frozenColumns;
		...
		for(var j=0;j<opt.length;j++){
			for(var i=0; i<opt[j].length; i++){
				if(opt[j][i].subtotal){
					footer['subtotal'] = footer['subtotal']+sum(opt[j][i].field)+',';
				}
				if(opt[j][i].subtitle){
					footer['subtitle']='"'+opt[j][i].field+'":"'+opt[j][i].subtitle+'"';
				}
			}
		}
		...
		if(footer['subtitle']==""){
			for(var i=0; i<forzenOpt[0].length; i++){
				if(forzenOpt[0][i].subtitle){
					footer['subtitle']='"'+forzenOpt[0][i].field+'":"'+forzenOpt[0][i].subtitle+'"';
					break;
				}
			}
		}
		if(footer['subtotal']!=""){
			var tmp='{'+footer['subtotal'].substring(0,footer['subtotal'].length-1)+"}";
			var obj=eval('('+tmp+')');
			obj = eval('({'+footer['subtitle']+','+footer['subtotal']+'})');
			footerObj.push(obj);
		}
		...
		function sum(filed){
			var sumNum=0;
			for(var i=0;i<rows.length;i++){
				sumNum+=Number(!isNaN(rows[i][filed])?rows[i][filed]:0.00);
			}
			return '"'+filed+'":"'+(!isNaN(sumNum.toFixed(2))?sumNum.toFixed(2):0.00)+'"';
		};
	}
});
...
$.ajaxSetup({ 
	...
	complete:function(XMLHttpRequest,textStatus){
		...
		if(sessionstatus=='timeout'){
			...
			top.location.href = "http://"+window.location.host+ctx+"/login!index.action"; //跳转到登陆页面 对多服务器同样适用
		}
	}
});
