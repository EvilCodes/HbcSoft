...
function TiJiao(ids){
	...
	for(var i=0; i<trList.length; i++){
		...
		var defaultValue = tdArr.eq(18).find("input").val();//默认值
		if(number==''){
			number = 0;
		}
		if(isNull=='否'){
												...
												<td>
													<div class="form-group" style="width: 100px">
														...
														<c:if test="${entity.inputType != '5' && entity.inputType != '7'}">
															<select class="form-control" name="inputType" title="${entity.inputType}" id="${entity.fieldId}a" onchange="test('${entity.fieldId}a','${entity.fieldId}b','${entity.fieldId}c','${entity.fieldId}d')">
